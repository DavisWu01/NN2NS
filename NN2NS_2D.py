...
class DNN2NS(tn.Module):
    def __init__(self, input_dim=2, out_dim=1, hidden_layer=None, Model_name='DNN', name2actIn='relu',
        ...
        if 'DNN' == str.upper(Model_name):
            self.DNN2Solu1 = DNN_base.Pure_DenseNet(
            ...
            self.DNN2Solu2 = DNN_base.Pure_DenseNet(
                indim=input_dim, outdim=out_dim, hidden_units=hidden_layer, name2Model=Model_name,
                actName2in=name2actIn, actName=name2actHidden, actName2out=name2actOut, type2float=type2numeric, scope2W='UW', scope2B='BU',
                to_gpu=use_gpu, gpu_no=No2GPU)
        elif 'SCALE_DNN' == str.upper(Model_name) or 'DNN_SCALE' == str.upper(Model_name):
            self.DNN2Solu1 = DNN_base.Dense_ScaleNet(
                indim=input_dim, outdim=out_dim, hidden_units=hidden_layer, name2Model=Model_name, actName2in=name2actIn,
                actName=name2actHidden, actName2out=name2actOut, type2float=type2numeric, scope2W='UW', scope2B='BU',
                repeat_Highfreq=repeat_highFreq2Solu, to_gpu=use_gpu, gpu_no=No2GPU)
            self.DNN2Solu2 = DNN_base.Dense_ScaleNet(
                indim=input_dim, outdim=out_dim, hidden_units=hidden_layer, name2Model=Model_name,
                actName2in=name2actIn,actName=name2actHidden, actName2out=name2actOut, type2float=type2numeric, scope2W='UW', scope2B='BU',
                repeat_Highfreq=repeat_highFreq2Solu, to_gpu=use_gpu, gpu_no=No2GPU)
        elif 'FOURIER_DNN' == str.upper(Model_name) or 'DNN_FOURIERBASE' == str.upper(Model_name):
            self.DNN2Solu1 = DNN_base.Dense_FourierNet(
                indim=input_dim, outdim=out_dim, hidden_units=hidden_layer, name2Model=Model_name, actName2in=name2actIn,
                actName=name2actHidden, actName2out=name2actOut, type2float=type2numeric, scope2W='UW', scope2B='BU',
                repeat_Highfreq=repeat_highFreq2Solu, to_gpu=use_gpu, gpu_no=No2GPU)
            self.DNN2Solu2 = DNN_base.Dense_FourierNet(
                indim=input_dim, outdim=out_dim, hidden_units=hidden_layer, name2Model=Model_name,
                actName2in=name2actIn,actName=name2actHidden, actName2out=name2actOut, type2float=type2numeric, scope2W='UW', scope2B='BU',
                repeat_Highfreq=repeat_highFreq2Solu, to_gpu=use_gpu, gpu_no=No2GPU)
    ...
    def loss_it(self, XY=None,
        ...
        if if_lambda2fside:
            ...
            force_side1 = fside1
            force_side2 = fside2
        UNN1 = self.DNN2Solu1(XY, scale=self.freq2Solu, sFourier=self.sFourier2Solu)
        UNN2 = self.DNN2Solu2(XY, scale=self.freq2Solu, sFourier=self.sFourier2Solu)
        ...
        if str.lower(loss_type) == 'l2_loss':
            ...
            return UNN1, UNN2, KNN, loss_in, loss_in2div
    ...
    def loss_bd(self, XY_bd=None,
        ...
        if if_lambda2Ubd:
            ...
            Ubd1 = Ubd_exact1
            Ubd2 = Ubd_exact2
        UNN1_bd = self.DNN2Solu1(XY_bd, scale=self.freq2Solu, sFourier=self.sFourier2Solu)
        UNN2_bd = self.DNN2Solu2(XY_bd, scale=self.freq2Solu, sFourier=self.sFourier2Solu)
    ...
    def loss_exact(self, XY=None,
        ...
        if if_lambda2Kexact:
            ...
            U1ture = U1exact
            U2ture = U2exact
        U1_exact = self.DNN2Solu1(XY,scale=self.freq2Solu, sFourier=self.sFourier2Solu)
        U2_exact = self.DNN2Solu2(XY, scale=self.freq2Solu, sFourier=self.sFourier2Solu)
    ...
    def get_regularSum2WB(self):
        sum2WB_Solu1 = self.DNN2Solu1.get_regular_sum2WB(self.opt2regular_WB)
        sum2WB_Solu2 = self.DNN2Solu2.get_regular_sum2WB(self.opt2regular_WB)
        sum2WB_Para = self.DNN2P.get_regular_sum2WB(self.opt2regular_WB)
        return sum2WB_Solu1 + sum2WB_Solu2 + sum2WB_Para

    def evalue_DNN2NS(self, XY_points=None):
        assert (XY_points is not None)
        UNNX = self.DNN2Solu1(XY_points, scale=self.freq2Solu, sFourier=self.sFourier2Solu)
        UNNY = self.DNN2Solu2(XY_points, scale=self.freq2Solu, sFourier=self.sFourier2Solu)
        KNN = self.DNN2P(XY_points, scale=self.freq2PModel, sFourier=self.sFourier2P)
        return UNNX, UNNY, KNN
...
def solve_PDE(R):
    ...
    if R['PDE_type'] == 'Navier_Stoke':
        f1 = lambda x, y: torch.zeros_like(x) #验算
    ...
    params2Net_11 = NS_dnn.DNN2Solu1.parameters()
    params2Net_12 = NS_dnn.DNN2Solu2.parameters()
    params2Net_2 = NS_dnn.DNN2P.parameters()

    params2Net = itertools.chain(params2Net_11, params2Net_12, params2Net_2)
    ...
    for i_epoch in range(R['max_epoch'] + 1):
        ...
        xl_bd_batch, xr_bd_batch, xt_bd_batch, xb_bd_batch = dataUtilizer2torch.rand_bd_2D(batchsize_bd, input_dim,
                                                                 region_a=region_l, region_b=region_r, to_torch=True, to_float=True,
        ...
        if R['PDE_type'] == 'Navier_Stoke':
            UNN2trainx, UNN2trainy, KNN2train, loss_it, loss_it2div = NS_dnn.loss_it(
        ...
        if i_epoch % 1000 == 0:
            ...
            # ---------------------------   test network ----------------------------------------------
            UNN2testx, UNN2testy, KNN2test = NS_dnn.evalue_DNN2NS(XY_points=xy_it2test_batch)
    ...
    saveData.save_2testSolus2mat(U1true2test, UNN2testx, actName='Solu_1', actName1='SoluNN',
                                 outPath=R['FolderName'])

    saveData.save_2testSolus2mat(U2true2test, UNN2testy, actName='Solu_1', actName1='SoluNN',
                                 outPath=R['FolderName'])
