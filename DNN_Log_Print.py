...
def dictionary_out2file(R_dic, log_fileout):
    ...
    if R_dic['input_dim'] == 1:
        ...
        if R_dic['PDE_type'] == 'pLaplace_implicit' or R_dic['PDE_type'] == 'pLaplace_explicit':
            ...
            DNN_tools.log_string('The mesh_number: %f\n' % (R_dic['mesh_number']), log_fileout)
        elif R_dic['PDE_type'] == 'Navier_Stoke':
            DNN_tools.log_string('The mesh_number: %f\n' % (R_dic['mesh_number']), log_fileout)
    else:
        DNN_tools.log_string('The order of pLaplace operator: %s\n' % (R_dic['order2pLaplace_operator']),
                             log_fileout)
...
def print_and_log_train_one_epoch2NS(i_epoch, run_time, tmp_lr, temp_penalty_bd, pwb, loss_it,
                loss_bd, loss, train_MSE2P,train_REL2P, train_MSE2Solu_1,
                train_REL2Solu_1, train_MSE2Solu_2, train_REL2Solu_2,log_out):
    print('train epoch: %d, time: %.3f' % (i_epoch, run_time))
    print('learning rate: %f' % tmp_lr)
    print('boundary penalty: %f' % temp_penalty_bd)
    print('weights and biases with  penalty: %f' % pwb)
    print('loss_it for training: %.10f' % loss_it)
    print('loss_bd for training: %.10f' % loss_bd)
    print('loss for training: %.10f' % loss)
    print('solution 1 mean square error for training: %.10f' % train_MSE2Solu_1)
    print('solution 1 residual error for training: %.10f' % train_REL2Solu_1)
    print('solution 2 mean square error for training: %.10f' % train_MSE2Solu_2)
    print('solution 2 residual error for training: %.10f' % train_REL2Solu_2)
    print('pressure mean square error for training: %.10f' % train_MSE2P)
    print('pressure residual error for training: %.10f\n' % train_REL2P)

    DNN_tools.log_string('train epoch: %d,time: %.3f' % (i_epoch, run_time), log_out)
    DNN_tools.log_string('learning rate: %f' % tmp_lr, log_out)
    DNN_tools.log_string('boundary penalty: %f' % temp_penalty_bd, log_out)
    DNN_tools.log_string('weights and biases with  penalty: %f' % pwb, log_out)
    DNN_tools.log_string('loss_it for training: %.10f' % loss_it, log_out)
    DNN_tools.log_string('loss_bd for training: %.10f' % loss_bd, log_out)
    DNN_tools.log_string('loss for training: %.10f' % loss, log_out)
    DNN_tools.log_string('solution 1 mean square error for training: %.10f' % train_MSE2Solu_1, log_out)
    DNN_tools.log_string('solution 1 residual error for training: %.10f' % train_REL2Solu_1, log_out)
    DNN_tools.log_string('solution 2 mean square error for training: %.10f' % train_MSE2Solu_2, log_out)
    DNN_tools.log_string('solution 2 residual error for training: %.10f' % train_REL2Solu_2, log_out)
    DNN_tools.log_string('pressure mean square error for training: %.10f' % train_MSE2P, log_out)
    DNN_tools.log_string('pressure residual error for training: %.10f\n\n' % train_REL2P, log_out)

def print_and_log_test_one_epoch2NS(mse2p, res2p, mse2test_sol1, res2test_sol1, mse2test_sol2, res2test_sol2, log_out=None):
    # 将运行结果打印出来
    print('solution 1 mean square error of predict and real for testing: %.10f' % mse2test_sol1)
    print('solution 1 residual error of predict and real for testing: %.10f' % res2test_sol1)
    print('solution 2 mean square error of predict and real for testing: %.10f' % mse2test_sol2)
    print('solution 2 residual error of predict and real for testing: %.10f' % res2test_sol2)
    print('pressure mean square error of predict and real for testing: %.10f' % mse2p)
    print('pressure residual error of predict and real for testing: %.10f\n' % res2p)


    DNN_tools.log_string('solution 1 mean square error of predict and real for testing: %.10f' % mse2test_sol1, log_out)
    DNN_tools.log_string('solution 1 residual error of predict and real for testing: %.10f' % res2test_sol1, log_out)
    DNN_tools.log_string('solution 2 mean square error of predict and real for testing: %.10f' % mse2test_sol2, log_out)
    DNN_tools.log_string('solution 2 residual error of predict and real for testing: %.10f' % res2test_sol2, log_out)
    DNN_tools.log_string('pressure mean square error of predict and real for testing: %.10f' % mse2p, log_out)
    DNN_tools.log_string('pressure residual error of predict and real for testing: %.10f\n\n' % res2p, log_out)
